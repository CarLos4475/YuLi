  ...
  Future<void> reorder(int noteId, List<int> orderedIds) async {
    await transaction(() async {
      for (int i = 0; i < orderedIds.length; i++) {
        await (update(noteBlocks)
              ..where((b) =>
                  b.id.equals(orderedIds[i]) & b.noteId.equals(noteId)))
            .write(NoteBlocksCompanion(
                position: Value(i), updatedAt: Value(DateTime.now())));
      }
    });
  }

  Future<int> getMaxPosition(int noteId) async {
    final rows = await (select(noteBlocks)
          ..where((b) => b.noteId.equals(noteId))
