...
class TasksDao extends DatabaseAccessor<AppDatabase> with _$TasksDaoMixin {
  ...
  Stream<List<TaskRow>> watchPending() =>
      (select(tasks)
            ..where((t) => t.status.equals('pending'))
            ..orderBy([(t) => OrderingTerm.asc(t.createdAt)]))
          .watch();

  Stream<List<TaskRow>> watchYesterday() =>
      (select(tasks)
            ..where((t) => t.status.equals('yesterday'))
            ..orderBy([(t) => OrderingTerm.asc(t.createdAt)]))
          .watch();

  Stream<List<TaskRow>> watchVencidas() =>
      (select(tasks)
  ...
  Future<List<TaskRow>> getPendingForFolder(int folderId) =>
      (select(tasks)..where(
        (t) => t.status.equals('pending') & t.folderId.equals(folderId),
      )).get();
  ...
  Future<void> markDone(int id) => (update(tasks)
  ...
  Future<void> unmarkDone(int id) => (update(tasks)
    ..where((t) => t.id.equals(id))).write(
    const TasksCompanion(status: Value('pending'), completedAt: Value(null)),
  );

  Future<void> rescueToday(int id) => (update(tasks)..where(
    (t) => t.id.equals(id),
  )).write(const TasksCompanion(status: Value('pending')));

  Future<void> moveToTrash(int id) => (update(tasks)
    ..where((t) => t.id.equals(id))).write(
    TasksCompanion(
  ...
  Stream<List<TaskRow>> watchTrashed() =>
      (select(tasks)
            ..where((t) => t.status.equals('trash'))
            ..orderBy([(t) => OrderingTerm.desc(t.trashedAt)]))
          .watch();

  Future<void> updateDueDate(int id, DateTime? dueDate) => (update(tasks)
    ..where(
      (t) => t.id.equals(id),
